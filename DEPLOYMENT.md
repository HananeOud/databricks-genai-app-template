...
This guide explains how to integrate the Assistant Template App into a host React application (specifically for `dbdemos-genai` integration).

## Overview

The Assistant Template App can be deployed as a compiled, standalone application within a host React app. The key feature is **runtime configuration** - the app reads its endpoint configuration from an `app_config.json` file at runtime, allowing the host app to dynamically configure available endpoints without rebuilding.
...
```bash
# Build and output to default location (./dist/assistant-app-app)
...
**Frontend (Host App):**
The Assistant Template App will automatically fetch `/app_config.json` on load.
...
./scripts/generate_prod_config.sh > dist/assistant-app-app/app_config.json

# Or for development
./scripts/generate_dev_config.sh > dist/assistant-app-app/app_config.json
```

## Backend API Requirements

The Assistant Template App requires the following API endpoints from the host:
...
```bash
# Copy to host app
cp -r ./dist/assistant-app-app/* /path/to/host-app/public/assistant-app/
...
open http://localhost:3000/assistant-app
```
...
1. **Option 1: CSS Override**
   ...
   ```html
   <link rel="stylesheet" href="/assistant-app/assets/index.css" />
